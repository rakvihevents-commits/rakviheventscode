...
export default function LoginModal({ isOpen, onClose }: LoginModalProps) {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (view === 'login') {
      ...
      // ✅ "Confirm email" is disabled in Supabase Auth settings, so
      // signUp returns an active session immediately — the user is
      // already logged in at this point, just like the login flow.
      // No "check your email" messaging needed.
      ...
      if (error) {
        ...
        // Confirmation is off — user is signed in right away.
        setMessage({ type: 'success', text: 'Account created! Redirecting...' });
        setTimeout(() => { onClose(); window.location.reload(); }, 1500);
      } else {
        // Fallback, in case confirmation gets re-enabled later.
